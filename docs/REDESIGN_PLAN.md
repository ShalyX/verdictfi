...
### Typography
- Display: `Source Serif 4` via `next/font/google`, variable `--font-display`; formal document gravity without the familiar Claude-artifact Fraunces feel.
- Body: `IBM Plex Sans` via `next/font/google`, variable `--font-body`; enterprise/technical clarity with more character than Inter.
- Evidence/data: `IBM Plex Mono`, variable `--font-evidence`; evidence IDs, hashes, timestamps, and audit metadata stay precise.
...
- Generic sans: **Source Serif 4 headlines + IBM Plex Mono labels/data + IBM Plex Sans body.**
- Trading terminal vibe: **rename sections toward evidence, case file, audit trail, case log; no leaderboard framing.**

## Files to change
- `src/app/layout.tsx` — load Source Serif 4, IBM Plex Sans, IBM Plex Mono; update metadata if needed.
...
- Typography exact? Source Serif 4 / IBM Plex Mono / IBM Plex Sans planned: yes.
